"""
Benchmark Comparativo de Híbridos Sequenciais no MNIST
======================================================
Script Principal para Execução

Este script importa:
- Otimizadores de 'optimizers.py'
- Modelos de 'models.py'
- Funções de utilidade de 'utils.py'
- Funções de plotagem de 'plotting.py'

E então executa os experimentos.
"""
...
from utils import get_data_loaders, run_experiment
from plotting import generate_nn_plots, generate_lr_plots
...
def main():
    ...
    print("\n" + "="*80)
    print("PARTE 1: EXPERIMENTOS COM REDES NEURAIS (HÍBRIDOS SEQUENCIAIS)")
    print("="*80)
    ...
    history_hybrid_sgd, time_hybrid_sgd = run_experiment(
        experiment_name="Híbrido (Adam -> SGD+M)",
    ...
    # --- EXPERIMENTO 3: Híbrido (Adam → SIGMA-D) ---
    ...
    history_hybrid_sigma_d, time_hybrid_sigma_d = run_experiment(
        experiment_name="Híbrido (Adam -> SIGMA-D)",
    ...
    results_nn['Adam -> SIGMA-D'] = history_hybrid_sigma_d
    times_nn['Adam -> SIGMA-D'] = time_hybrid_sigma_d

    # --- EXPERIMENTO 4: Híbrido (Adam → SIGMA-C) ---
    ...
    history_hybrid_sigma_c, time_hybrid_sigma_c = run_experiment(
        experiment_name="Híbrido (Adam -> SIGMA-C)",
    ...
    results_nn['Adam -> SIGMA-C'] = history_hybrid_sigma_c
    times_nn['Adam -> SIGMA-C'] = time_hybrid_sigma_c
    ...
    results_lr['Adam -> SIGMA-C'] = history_lr_h_sigmac
    times_lr['Adam -> SIGMA-C'] = time_lr_h_sigmac
    ...
    print("\n" + "-"*80)
    print("ANÁLISE DE MELHORIAS (vs Híbrido SGD)")
    print("-"*80)
    
    baseline_acc_nn = results_nn['Adam -> SGD+M']['test_acc'][-1]
    acc_d_nn = results_nn['Adam -> SIGMA-D']['test_acc'][-1]
    acc_c_nn = results_nn['Adam -> SIGMA-C']['test_acc'][-1]
    
    diff_d_nn = acc_d_nn - baseline_acc_nn
    diff_c_nn = acc_c_nn - baseline_acc_nn
    
    print(f"Híbrido (Adam -> SIGMA-D) vs (Adam -> SGD+M): {diff_d_nn:+.2f}% {'✓' if diff_d_nn > 0 else '✗'}")
    print(f"Híbrido (Adam -> SIGMA-C) vs (Adam -> SGD+M): {diff_c_nn:+.2f}% {'✓' if diff_c_nn > 0 else '✗'}")
    ...
    for name in results_lr.keys():
        ...
        loss_final = results_lr[name]['train_loss'][-1] # Loss de treino é mais estável em LR
    ...
    acc_d_lr = results_lr['Adam -> SIGMA-D']['test_acc'][-1]
    acc_c_lr = results_lr['Adam -> SIGMA-C']['test_acc'][-1]
    
    diff_d_lr = acc_d_lr - baseline_acc_lr
    diff_c_lr = acc_c_lr - baseline_acc_lr
    
    print("Comparação Híbridos (vs Híbrido SGD):")
    print(f"Híbrido (Adam -> SIGMA-D) vs (Adam -> SGD+M): {diff_d_lr:+.2f}% {'✓' if diff_d_lr > 0 else '✗'}")
    print(f"Híbrido (Adam -> SIGMA-C) vs (Adam -> SGD+M): {diff_c_lr:+.2f}% {'✓' if diff_c_lr > 0 else '✗'}")
