...
def generate_nn_plots(results_nn, times_nn, n_epochs_phase1):
    ...
    Salva como 'sigma_hybrid_comparison_nn.pdf'.
    """
    ...
    ax1.set_title('Comparação de Acurácia: Híbridos vs Baseline (Redes Neurais)', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('sigma_hybrid_comparison_nn.pdf', dpi=150, bbox_inches='tight') # Salvo como .pdf
    print("Gráfico salvo: 'sigma_hybrid_comparison_nn.pdf'")
...
def generate_lr_plots(results_lr, times_lr, n_epochs_phase1):
    ...
    Salva como 'sigma_full_comparison_logistic.pdf'.
    """
    
    print("\nGerando gráficos de comparação (Regressão Logística)...")
    
    # --- MODIFICADO: fig, axes = plt.subplots(1, 2,...) -> plt.subplots(2, 2,...) ---
    ...
    # --- Plot 2: Loss de Teste (axes2[0, 1]) --- (NOVO PARA LR)
    ax2_lr = axes2[0, 1]
    for name, history in results_lr.items():
    ...
    ax2_lr.set_title('Regressão Logística: Loss de Teste', fontsize=14, fontweight='bold')
    ...
    ax3_lr.set_title('Regressão Logística: Convergência', fontsize=14, fontweight='bold')
    ax3_lr.set_yscale('log')
    ax3_lr.legend(fontsize=9, loc='upper right')
    ax3_lr.grid(True, alpha=0.3)

    # --- Plot 4: Tempo (axes2[1, 1]) --- (NOVO PARA LR)
    ax4_lr = axes2[1, 1]
    ...
    ax4_lr.set_title('Eficiência Computacional (Regressão Logística)', fontsize=14, fontweight='bold')
    ax4_lr.grid(axis='x', alpha=0.3)

    
    plt.tight_layout()
    plt.savefig('sigma_full_comparison_logistic.pdf', dpi=150, bbox_inches='tight') # Salvo como .pdf
    print("Gráfico salvo: 'sigma_full_comparison_logistic.pdf'")