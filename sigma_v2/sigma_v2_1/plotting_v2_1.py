...
def generate_nn_plots(results_nn, times_nn, n_epochs_phase1):
    ...
    print("\nGerando gráficos de comparação (Redes Neurais - v2.1)...")
    
    fig, axes = plt.subplots(2, 2, figsize=(18, 12))
    
    # *** ATUALIZADO: Novos nomes e cores ***
    colors_nn = {
        'Adam (Baseline)': '#1f77b4',
        'Adam -> SGD+M (Classic)': '#d62728',
        'Adam -> SGD+M (Nesterov)': '#ff7f0e',
        "Adam -> SIGMA-C (Momentum='sigma')": '#9467bd',
        "Adam -> SIGMA-C (Momentum='classic')": '#8c564b',
        "Adam -> SIGMA-C (Momentum='nesterov')": '#e377c2',
    }
    
    markers_nn = {
        'Adam (Baseline)': 'o',
        'Adam -> SGD+M (Classic)': 'v',
        'Adam -> SGD+M (Nesterov)': 'v',
        "Adam -> SIGMA-C (Momentum='sigma')": 'P',
        "Adam -> SIGMA-C (Momentum='classic')": 's',
        "Adam -> SIGMA-C (Momentum='nesterov')": 's',
    }
    ...
    linestyles_nn = {
        ...
        'Adam -> SGD+M (Classic)': ':',
        'Adam -> SGD+M (Nesterov)': '-',
        "Adam -> SIGMA-C (Momentum='sigma')": '-',
        "Adam -> SIGMA-C (Momentum='classic')": ':',
        "Adam -> SIGMA-C (Momentum='nesterov')": '--',
    }
    
    # --- Plot 1: Acurácia no Teste ---
    ax1 = axes[0, 0]
    for name, history in results_nn.items():
        if name not in colors_nn: continue
        ax1.plot(history['test_acc'], 
                label=name, 
                color=colors_nn[name],
                marker=markers_nn[name],
                linestyle=linestyles_nn[name],
                markersize=4,
                linewidth=2)
    ...
    ax1.set_title('Comparação de Acurácia: Modos de Momentum (Redes Neurais)', fontsize=14, fontweight='bold')
    ...
    ax3.set_title('Convergência no Treino (Redes Neurais)', fontsize=14, fontweight='bold')
...
def generate_lr_plots(results_lr, times_lr, n_epochs_phase1):
    ...
    print("\nGerando gráficos de comparação (Regressão Logística - v2.1)...")
    ...
    for name, history in results_lr.items():
        if name not in colors_lr: continue
    ...
    ax1_lr.set_title('Regressão Logística v2.1: Acurácia (Problema Convexo)', fontsize=14, fontweight='bold')
    ...
    ax2_lr.set_title('Regressão Logística v2.1: Loss de Teste', fontsize=14, fontweight='bold')
    ...
    ax3_lr.set_title('Regressão Logística v2.1: Convergência', fontsize=14, fontweight='bold')
    ...
    ax4_lr.set_title('Eficiência Computacional v2.1 (Regressão Logística)', fontsize=14, fontweight='bold')
